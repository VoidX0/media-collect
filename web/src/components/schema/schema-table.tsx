...
import { formatMillisecond } from '@/lib/date-time'
...
export default function SchemaTable<T extends Record<string, unknown>>({
  ...
  // 当 schema 变化时，重置列顺序
  useEffect(() => {
    setColumnOrder(Object.keys(schema) as (keyof T)[])
  }, [schema])
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="custom-scrollbar grid w-full grid-cols-[minmax(0,1fr)] [&>div]:max-h-(--table-max-height) [&>div]:overflow-auto"
          style={
            {
              '--table-max-height':
          ...
          <Table className="min-w-full">
            <TableHeader className="bg-background sticky top-0 z-20 shadow-[0_1px_0_0_hsl(var(--border))]">
              <TableRow className="hover:bg-background">
                {/*选中列*/}
                <TableHead className="bg-background w-12.5">
                  <Checkbox
                    checked={
                      allSelected
                        ? true
            ...
            <TableBody>
              {tableData.map((row, rowIndex) => {
                // 检查原始数据(data)中对应索引的行是否为空对象
                // 如果 data[rowIndex] 是 {}，说明真实数据还没加载
                const isPlaceholder =
                  isLoadingData ||
                  !data[rowIndex] ||
                  Object.keys(data[rowIndex]).length === 0
                  ...
                  return (
                    <TableRow key={`skeleton-${rowIndex}`}>
                      ...
                      {renderedColumns.map((col) => (
                        ...
                        >
                          {/* 给骨架屏一个跟文字差不多的小高度， 并且加上 whitespace-nowrap 保持列宽不塌缩 */}
                    ...
                    {renderedColumns.map((col) => {
                      ...
                      return (
                        <TableCell key={String(col)}>
                          {isDateTime ? (
                            formatMillisecond(new Date(value as number))
                          ) : isBoolean ? (
