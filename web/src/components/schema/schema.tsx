...
interface SchemaProps<T extends Record<string, unknown>> {
  ...
  fields?: SchemaType
  /** 在默认请求之前修改查询参数 */
  beforeQuery?: (query: QueryDto) => QueryDto
  /** 查询分页数据回调 */
  fetchPageData?: (dto: QueryDto) => Promise<{ items: T[]; total: number }>
  /** 新增回调 */
  ...
  /** 查询参数变更回调（受控模式） */
  onQueryDtoChange?: (dto: QueryDto) => void
}
...
const Schema = forwardRef(function Schema<T extends Record<string, unknown>>(
  ...
  // 查询条件缓存
  const cachedQueryRef = useRef<Record<string, string>>({})
  // 正在加载数据
  const [isLoadingData, setIsLoadingData] = useState(false)
  // 当前页码
  const [currentPage, setCurrentPage] = useState(1)
  // 查询参数（支持受控 / 非受控）
  const isControlled = props.queryDto !== undefined
  const [innerQueryDto, setInnerQueryDto] = useState<QueryDto>({})
  const queryDto = isControlled ? props.queryDto! : innerQueryDto
  const setQueryDto = isControlled ? props.onQueryDtoChange! : setInnerQueryDto
  // 所有数据
  const [data, setData] = useState<T[]>([])
  // 字段配置（优先使用 props.fields，其次使用 schemas 中的定义）
  const fieldMap = useMemo(() => {
    if (props.fields) {
      return props.fields
    }
    return schemas[typeName] || {}
  }, [props.fields, typeName])
  // 当前显示列
  const [visibleColumns, setVisibleColumns] = useState<string[]>(
    // 默认隐藏以$开头的字段
    Object.keys(fieldMap).filter((key) => !key.startsWith('$')),
  )
  // 显示的数据
  ...
  /** 字段配置变化时，重置显示列和选择 */
  useEffect(() => {
    const nextColumns = Object.keys(fieldMap).filter(
      (key) => !key.startsWith('$'),
    )
    setVisibleColumns(nextColumns)
    setSelectedKeys([]) // 字段切换时，清空选择
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (beforeQuery) {
        body = beforeQuery(body)
      }

      // 查询缓存,如果启用且存在缓存,跳过查询
      if (cacheQuery) {
        const cacheKey = `${body.pageNumber}-${body.pageSize}`
        if (
          cachedQueryRef.current[cacheKey] &&
          cachedQueryRef.current[cacheKey] === JSON.stringify(body)
        ) {
          return
        }
      }
      ...
      if (fetchPageData) {
        // 自定义回调
        const res = await fetchPageData(body)
      ...
      // 向查询缓存中保存当前查询条件
      if (cacheQuery && pagedItems.length > 0) {
        const cacheKey = `${body.pageNumber}-${body.pageSize}`
        cachedQueryRef.current[cacheKey] = JSON.stringify(body)
      }
  ...
  const handleDownload = async () => {
    if (props.onDownload) {
      ...
      if (data && response) {
        ...
        saveAs(data as unknown as Blob, filename)
      }
    }
  }
  ...
  return (
    <Card className="grid w-full grid-cols-[minmax(0,1fr)] border-none bg-transparent p-0 shadow-none">
      <MagicCard
      ...
      >
        ...
        <div className="grid w-full grid-cols-[minmax(0,1fr)]">
          <SchemaTable
            ...
            isLoadingData={showSkeleton && isLoadingData}
