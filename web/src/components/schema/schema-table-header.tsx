import { QueryCondition, QueryOrder } from '@/api/generatedSchemas'
import { SchemaType } from '@/components/schema/schema'
import { SchemaAi } from '@/components/schema/schema-ai'
...
export function SchemaTableHeader<T extends Record<string, unknown>>({
  ...
  const [aiOpen, setAiOpen] = useState(false) // 控制AI分析Dialog状态
  ...
  return (
    <div className="mt-4 flex w-full flex-col items-center gap-3 md:flex-row md:items-center md:justify-between">
      ...
      {/* 中间区域 */}
      <div className="order-first flex-1 text-center text-lg font-semibold md:order-0">
        {title}
      </div>
      ...
      <div className="flex flex-1 justify-end gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            ...
            <DropdownMenuItem onClick={() => setAiOpen(true)} hidden={true}>
              <Bot />
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="bg-background max-h-[90vh] w-full max-w-[90vw] overflow-y-auto border-none p-0 shadow-none md:max-w-5xl lg:max-w-7xl">
