...
WORKDIR /src
COPY ./api/ ./
RUN dotnet restore "MediaCollect/MediaCollect.csproj"
# 编译
WORKDIR "/src/MediaCollect"
RUN dotnet build "MediaCollect.csproj" -c $BUILD_CONFIGURATION -o /app/build
# 发布
FROM api_build AS api_publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "MediaCollect.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
WORKDIR /app
EXPOSE 8080 3000
LABEL org.opencontainers.image.source=https://github.com/VoidX0/media-collect
# 安装工具
