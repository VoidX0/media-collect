...
public static class SqlSugarExtension
{
    ...
    extension(ISqlSugarClient db)
    {
        ...
        public void InitDb(Action<string>? initSucceeded = null, Action<Exception, string>? initFailed = null)
        {
            try
            {
                //建库
        ...
        public void InitSystem(string defaultUser, string defaultPassword,
            ...
            Action<string>? initSucceeded = null, Action<Exception, string>? initFailed = null)
        {
            ...
            {
                //检查平台管理员用户
                if (!db.Queryable<SystemUser>().ToList().Any(x => x.IsPlatformAdmin))
