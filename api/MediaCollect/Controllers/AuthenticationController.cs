...
using MediaCollect.Core.Utils.Security;
...
public class AuthenticationController : ControllerBase
{
    ...
    public AuthenticationController(IOptions<SystemOptions> systemOptions, IOptions<TokenOptions> tokenOptions,
        IOptions<OidcOptions> oidcOptions, SmtpService smtpService)
    {
    ...
    public async Task<ActionResult> SendVerifyCode(SystemUser dto, UserVerifyType verifyType)
    {
        ...
        if (isRegister) return BadRequest(MessageCodeEnum.RegisterNotOpen.ToMessageCode());
        //获取验证账号
        ...
        if (string.IsNullOrWhiteSpace(account)) return BadRequest(MessageCodeEnum.AccountRequired.ToMessageCode());
        //邮箱格式验证
        if (verifyType == UserVerifyType.Email && !IsEmail(dto.Email))
            return BadRequest(MessageCodeEnum.EmailFormatError.ToMessageCode());
        //查找验证码
        ...
        if (lastVerify != null && now - lastVerify.CreateTime < TimeSpan.FromMinutes(3))
            return BadRequest(MessageCodeEnum.VerifyCodeFrequent.ToMessageCode());
        //检查所有验证码，是否超过限制
        if (verify.Count >= 5) return BadRequest(MessageCodeEnum.VerifyCodeLimit.ToMessageCode(5));
        // 发送验证码
        var code = GenVerifyCode();
        var send = verifyType switch
        {
            UserVerifyType.Email => await _smtpService.SendMessage(
    ...
    public async Task<ActionResult<string>> Login(SystemUser dto)
    {
        //获取用户
        var user = await Db.Queryable<SystemUser>().FirstAsync(x => x.Email == dto.Email);
        if (user is null) return BadRequest(MessageCodeEnum.UserNotFound.ToMessageCode());
        //检查密码
        ...
        if (verify is null || verify.VerifyValue != dto.Password)
            return BadRequest(MessageCodeEnum.PasswordError.ToMessageCode());
        //更新验证码
    ...
    public async Task<ActionResult> SetAvatar(IFormFile file)
    {
        ...
        if (currentUser is null) return BadRequest(MessageCodeEnum.Unauthorized.ToMessageCode());
        switch (file.Length)
        {
            case 0:
                return BadRequest(MessageCodeEnum.FileEmpty.ToMessageCode());
            // 限制文件大小 10MB
            case > 10 * 1024 * 1024:
                return BadRequest(MessageCodeEnum.FileSizeLimit.ToMessageCode("10MB"));
        }
    ...
    public async Task<ActionResult> AddRole(SystemRole dto)
    {
        ...
        if (user is null or { IsPlatformAdmin: false })
            return BadRequest(MessageCodeEnum.NoPermissionAction.ToMessageCode("Add Role"));
        //查找角色
        if (await Db.Queryable<SystemRole>().FirstAsync(x => x.Name == dto.Name) != null)
            return BadRequest(MessageCodeEnum.EntityExists.ToMessageCode());
        //添加角色
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
            dto.Id = 0;
    ...
    /// <returns></returns>
    [HttpPost]
    public async Task<ActionResult> AddUser(SystemUser dto)
    {
        var user = HttpContext.User.Claims.Parsing();
        if (user is null or { IsPlatformAdmin: false })
            return BadRequest(MessageCodeEnum.NoPermissionAction.ToMessageCode("Add User"));
        //邮箱格式验证
        if (!IsEmail(dto.Email)) return BadRequest(MessageCodeEnum.EmailFormatError.ToMessageCode());
        //查找用户
        if (await Db.Queryable<SystemUser>().FirstAsync(x => x.Email == dto.Email) != null)
            return BadRequest(MessageCodeEnum.EmailExists.ToMessageCode());
        //rsa解密
        var password = SecurityRsa.Decrypt(dto.Password);
        if (string.IsNullOrWhiteSpace(password)) return BadRequest(MessageCodeEnum.DecryptError.ToMessageCode());
        //检查角色是否合法
        if (dto.Role.Count > 0)
        {
            // 查找所有在传入列表中的角色ID
            var validRoleIds = await Db.Queryable<SystemRole>()
                .Where(x => dto.Role.Contains(x.Id))
                .Select(x => x.Id)
                .ToListAsync();

            // 有效数量不一致，说明夹带了非法角色ID
            if (validRoleIds.Count != dto.Role.Count)
                return BadRequest(MessageCodeEnum.InvalidEntity.ToMessageCode("ID"));
        }
        ...
        {
            dto.Id = 0;
            dto.Password = SecurityAes.Encrypt(password);
            if (!user.IsPlatformAdmin)
            {
                dto.IsPlatformAdmin = false; // 不能添加平台管理员
            }
    ...
    public async Task<ActionResult> AddRoleGrant(long roleId, long controllerId)
    {
        var user = HttpContext.User.Claims.Parsing();
        if (user is null or { IsPlatformAdmin: false })
            return BadRequest(MessageCodeEnum.NoPermissionAction.ToMessageCode("Grant"));
        //检查角色和控制器是否存在
        var role = await Db.Queryable<SystemRole>().InSingleAsync(roleId);
        if (role == null)
            return BadRequest(MessageCodeEnum.RoleNotFound.ToMessageCode());
        var controller = await Db.Queryable<SystemController>().InSingleAsync(controllerId);
        if (controller == null)
            return BadRequest(MessageCodeEnum.ControllerNotFound.ToMessageCode());
        if (await Db.Queryable<SystemGrantRole>().AnyAsync(x => x.RoleId == roleId && x.ControllerId == controllerId))
            return BadRequest(MessageCodeEnum.GrantExists.ToMessageCode());
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
    ...
    public async Task<ActionResult> AddUserGrant(long userId, long controllerId)
    {
        ...
        if (user is null or { IsPlatformAdmin: false })
            return BadRequest(MessageCodeEnum.NoPermissionAction.ToMessageCode("Grant"));
        //检查用户和控制器是否存在
        var targetUser = await Db.Queryable<SystemUser>().InSingleAsync(userId);
        if (targetUser == null)
            return BadRequest(MessageCodeEnum.UserNotFound.ToMessageCode());
        var controller = await Db.Queryable<SystemController>().InSingleAsync(controllerId);
        if (controller == null)
            return BadRequest(MessageCodeEnum.ControllerNotFound.ToMessageCode());
    ...
    public async Task<ActionResult> DeleteRole(long roleId)
    {
        //获取角色相关信息
        var role = await Db.Queryable<SystemRole>().InSingleAsync(roleId);
        if (role is null) return BadRequest(MessageCodeEnum.RoleNotFound.ToMessageCode());
    ...
    {
        var user = HttpContext.User.Claims.Parsing();
        if (user is null) return BadRequest(MessageCodeEnum.Unauthorized.ToMessageCode());
        //获取用户
        var deleteUser = await Db.Queryable<SystemUser>().InSingleAsync(userId);
        if (deleteUser is null) return BadRequest(MessageCodeEnum.UserNotFound.ToMessageCode());
        //检查
        if (user.Id == deleteUser.Id) return BadRequest(MessageCodeEnum.DeleteSelfForbidden.ToMessageCode());
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
    ...
    public async Task<ActionResult> DeleteRoleGrant(long roleId, long controllerId)
    {
        ...
        if (grant == null) return BadRequest(MessageCodeEnum.GrantNotFound.ToMessageCode());
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
    ...
    public async Task<ActionResult> ModifyRole(SystemRole dto)
    {
        ...
        if (role == null) return BadRequest(MessageCodeEnum.RoleNotFound.ToMessageCode());
        //修改角色
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
            role.Name = dto.Name;
    ...
    public async Task<ActionResult> ModifyUser(SystemUser dto)
    {
        ...
        if (user == null) return BadRequest(MessageCodeEnum.UserNotFound.ToMessageCode());
        //检查权限
        var currentUser = HttpContext.User.Claims.Parsing();
        if (user.Id != currentUser?.Id && currentUser?.IsPlatformAdmin != true)
            return BadRequest(MessageCodeEnum.NoPermissionAction.ToMessageCode("Modify"));
        //邮箱格式验证
        if (!IsEmail(dto.Email)) return BadRequest(MessageCodeEnum.EmailFormatError.ToMessageCode());
        //查找邮箱
    ...
    public async Task<ActionResult<SystemUser>> CurrentUser()
    {
        ...
        if (dbUser is null) return BadRequest(MessageCodeEnum.UserNotFound.ToMessageCode());
    ...
    public async Task<IActionResult> GetAvatar(string id, string token)
    {
        ...
        if (!isValid) return BadRequest(MessageCodeEnum.TokenInvalidOrExpired.ToMessageCode());
        // 验证id
        if (!long.TryParse(id, out var userId)) return BadRequest(MessageCodeEnum.InvalidId.ToMessageCode());
        // 查找头像文件
        var avatarDir = Path.Combine(_systemOptions.StoragePath, "Avatar");
        var jpgPath = Path.Combine(avatarDir, $"{userId}.jpg");
        var pngPath = Path.Combine(avatarDir, $"{userId}.png");
        string? filePath = null;
        if (System.IO.File.Exists(jpgPath)) filePath = jpgPath;
        else if (System.IO.File.Exists(pngPath)) filePath = pngPath;
        if (filePath == null) return BadRequest(MessageCodeEnum.AvatarNotFound.ToMessageCode());
        // 返回文件
    ...
    public async Task<ActionResult<List<SystemController>>> Granted()
    {
        ...
        if (user is null) return BadRequest(MessageCodeEnum.Unauthorized.ToMessageCode());
        //平台管理员
        var controllers = await Db.Queryable<SystemController>().OrderBy(x => x.Id).ToListAsync();
        if (user.IsPlatformAdmin) return Ok(controllers);
        // 普通用户
        var roleKeys = await Db.Queryable<SystemGrantRole>()
            .Where(x => user.Role.Contains(x.RoleId))
            .Select(x => x.ControllerId)
            .ToListAsync();
        var userKeys = await Db.Queryable<SystemGrantUser>()
            .Where(x => x.UserId == user.Id)
            .Select(x => x.ControllerId)
            .ToListAsync();
        var ownedKeys = roleKeys.Union(userKeys).Distinct().ToList();
        var finalKeys = ownedKeys.Intersect(controllers.Select(x => x.Id)).ToList(); // 用户实际拥有的控制器
