...
using MediaCollect.Utils;
using Microsoft.AspNetCore.Mvc;
using SqlSugar;
using SqlSugar.IOC;

namespace MediaCollect.Controllers.Base;
...
public abstract class OrmController<T> : ControllerBase where T : class, new()
{
    ...
    /// <returns></returns>
    protected virtual ISugarQueryable<T> ApplyQueryFilter(ISugarQueryable<T> query) => query;
    ...
    protected virtual ISugarQueryable<T> BuildQuery(List<IConditionalModel> conditions)
    {
        ...
        query = ApplyQueryFilter(query); // 应用查询过滤器
        query = query.Where(otherConditions); // 处理其他条件
        // Range条件单独处理
    ...
    public virtual async Task<ActionResult<List<long>>> Insert(List<T> entity)
    {
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
            ...
            return IsSplitTable
                ? await insert.SplitTable().ExecuteReturnSnowflakeIdListAsync()
                : await insert.ExecuteReturnSnowflakeIdListAsync();
        });
    ...
    public virtual async Task<ActionResult<int>> Delete(List<T> entity)
    {
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
            var delete = Db.Deleteable(entity);
            return IsSplitTable
                ? await delete.SplitTable().ExecuteCommandAsync()
    ...
    public virtual async Task<ActionResult<int>> Update(List<T> entity)
    {
        var result = await Db.AsTenant().UseTranAsync(async () =>
        {
            ...
            return IsSplitTable
                ? await update.SplitTable().ExecuteCommandAsync()
    ...
    public virtual async Task<ActionResult<PagedResult<T>>> Query(QueryDto dto)
    {
        ...
        if (!conditions.IsSuccess || !orders.IsSuccess || conditions.Content is null || orders.Content is null)
            return BadRequest(
                MessageCodeEnum.ParamError.ToMessageCode($"{conditions.Message} {orders.Message}".Trim()));
        // 构建查询
    ...
    {
        var conditions = dto.GetConditions(Db);
        if (!conditions.IsSuccess || conditions.Content is null)
            return BadRequest(MessageCodeEnum.ParamError.ToMessageCode(conditions.Message));
        // 构建查询
    ...
    public virtual async Task<ActionResult> DownloadExcel(QueryDto dto)
    {
        ...
        var list = await BuildQuery(conditions.Content)
            ...
            .ToListAsync();
        if (list == null || list.Count == 0) return BadRequest(MessageCodeEnum.ExportDataEmpty.ToMessageCode());
        // 导出Excel
        var stream = ExcelOperation.Serialize(list);
        if (stream is null) return BadRequest(MessageCodeEnum.ExcelExportFailed.ToMessageCode());
