...
using System.Text;
using MediaCollect.Controllers;
...
internal sealed class MarkdownDescriptionTransformer : IOpenApiDocumentTransformer
{
    ...
    {
        // 获取所有控制器
        var controllers = typeof(AuthenticationController).Assembly.GetTypes()
            .Where(x => x is { IsPublic: true, IsClass: true, IsAbstract: false }
                        && x.Namespace?.StartsWith(typeof(AuthenticationController).Namespace ?? "unknown namespace") ==
                        true) //命名空间过滤
        ...
        foreach (var controller in controllers)
        {
            var methods = controller.GetMethods(BindingFlags.Instance | BindingFlags.Public)
                ...
                .ToList();
            // 获取控制器显示名称
            var descriptionAttrs = controller.GetCustomAttributes(typeof(DescriptionAttribute), false);
            var description = descriptionAttrs.Length > 0
                ? ((DescriptionAttribute)descriptionAttrs[0]).Description
                : string.Empty;
            var methodCount = 0;
            // 获取授权方式
            ...
            dict.Add([controller.Name, auth, description], methodCount);
        }
        ...
        sb.AppendLine("| Controller | Authorization | Description | Method |");
        sb.AppendLine("|------|-------|-------|-------|");
...
/// 动态解析服务器地址
/// </summary>
internal sealed class DynamicServerTransformer(IHttpContextAccessor httpContextAccessor) : IOpenApiDocumentTransformer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // 获取refer
        var referer = headers.Referer.FirstOrDefault() ?? string.Empty;
        const string keyword = "/api/";
        if (!string.IsNullOrEmpty(referer) && !referer.Contains(keyword))
        {
            // 重写 Servers 列表
            var slice = referer.Split(keyword);
            if (slice.Length <= 1) return Task.CompletedTask;
        ...
        // 获取x-forwarded-host
        var scheme = headers["x-forwarded-proto"].FirstOrDefault();
        var host = headers["x-forwarded-host"].FirstOrDefault();
...
/// 控制器描述
/// </summary>
internal sealed class ControllerDescriptionTransformer : IOpenApiDocumentTransformer
{
    /// <summary>
    /// TransformAsync
    /// </summary>
    ...
    {
        // 获取当前文档中包含的所有控制器信息
        var controllerDescriptors = context.DescriptionGroups
            .SelectMany(group => group.Items)
            .Select(apiDesc => apiDesc.ActionDescriptor as ControllerActionDescriptor)
            .Where(descriptor => descriptor != null)
            .DistinctBy(descriptor => descriptor!.ControllerTypeInfo); // 根据 ControllerType

        // 遍历控制器，提取 Description
        foreach (var descriptor in controllerDescriptors)
        {
            if (descriptor == null) continue;
            var controllerType = descriptor.ControllerTypeInfo;
            // 获取Description特性
            var descriptionAttr = controllerType.GetCustomAttribute<DescriptionAttribute>();
            if (descriptionAttr == null) continue;
            // 获取默认的 Tag 名称
            var tagName = descriptor.ControllerName;
            // 在文档的 Tags 列表中查找或创建对应的 Tag
            var tag = document.Tags?.FirstOrDefault(x => x.Name == tagName);
            if (tag == null)
            {
                tag = new OpenApiTag { Name = tagName };
                document.Tags?.Add(tag);
            }

            // 设置描述信息
            tag.Description = descriptionAttr.Description;
        }
...
internal sealed class EnumDescriptionTransformer : IOpenApiSchemaTransformer
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var isMessageCodeEnum = enumType.Name == "MessageCodeEnum"; // 特殊处理，直接显示名称而不是数值
        // 获取枚举的底层类型（int/long等）
        ...
        foreach (var name in Enum.GetNames(enumType))
        {
            ...
            // 添加
            var val = isMessageCodeEnum ? name : numericValue;
            sb.AppendLine($"| {val} | {desc ?? name} |");
        }

        // 写入描述字段
        schema.Description += sb.ToString();
        // 特殊处理，显示名称而不是数值
        if (isMessageCodeEnum) schema.Type = JsonSchemaType.String;
    }
}
