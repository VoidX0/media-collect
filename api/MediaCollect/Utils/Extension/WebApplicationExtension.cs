        ...
        public void Serilog()
        {
            ...
            app.UseSerilogRequestLogging(x =>
            {
                ...
                x.EnrichDiagnosticContext = (diagnosticContext, httpContext) =>
                {
                    ...
                    // 设置Query
                    const int queryLength = 512;
                    var requestQuery = HttpUtility.UrlDecode(httpContext.Request.QueryString.ToString());
                    if (requestQuery is { Length: > queryLength })
                        requestQuery = requestQuery[..queryLength] + "..."; // 限制Query长度
                    ...
                    if (requestBody != null && requestBody.ToString()?.Length > bodyLength)
                        requestBody = requestBody.ToString()?[..bodyLength] + "..."; // 限制Body长度
