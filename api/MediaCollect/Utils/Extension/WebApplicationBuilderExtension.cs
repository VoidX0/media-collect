...
public static class WebApplicationBuilderExtension
{
    ...
    extension(WebApplicationBuilder builder)
    {
        ...
        /// <returns></returns>
        private T GetConfiguration<T>() where T : class, new()
        {
            // 优先从缓存中获取
        ...
        public void ConfigureLog()
        {
            ...
            var configuration = new LoggerConfiguration()
                .Filter.ByExcluding(x =>
                    x.RenderMessage().Contains("/scalar/", StringComparison.CurrentCultureIgnoreCase) ||
                    x.RenderMessage().Contains("/openapi/", StringComparison.CurrentCultureIgnoreCase)) //排除的日志
        ...
        /// 配置SerilogUI
        /// </summary>
        public void ConfigureSerilogUi()
        {
            var system = builder.GetConfiguration<SystemOptions>();
        ...
        /// 配置API文档
        /// </summary>
        public void ConfigureApiReference()
        {
            ...
            builder.Services.AddOpenApi(x =>
            {
                ...
                x.AddDocumentTransformer<DynamicServerTransformer>();
                x.AddDocumentTransformer<BearerSecurityTransformer>();
                x.AddDocumentTransformer<ControllerDescriptionTransformer>();
            ...
            var controllers = typeof(AuthenticationController).Assembly.GetTypes()
                .Where(x => x is { IsPublic: true, IsClass: true, IsAbstract: false }
                            && x.Namespace?.StartsWith(
                                typeof(AuthenticationController).Namespace ?? "unknown namespace") ==
                            true) //命名空间过滤
                .Where(x => x.GetCustomAttributes(typeof(DescriptionAttribute), false).Length > 0)
                .Select(x => new SystemController
                {
                    Controller = x.Name,
                    Title = ((DescriptionAttribute)x.GetCustomAttributes(typeof(DescriptionAttribute), false)[0])
                        .Description
                }).ToList();
